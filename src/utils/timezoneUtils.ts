...
// Get all available timezones from the browser's Intl API
const getAllTimezones = (): string[] => {
  try {
    // Type assertion for supportedValuesOf which is available in newer browsers
...
const formatTimezoneLabel = (timezone: string): string => {
  ...
  try {
    // Get current time in the timezone to show offset
    const now = new Date();
    ...
    return `(UTC${offsetString}) ${cityName} - ${timeZoneName}`;
  } catch (error) {
    // Fallback formatting
    const cityName = timezone.split('/').pop()?.replace(/_/g, ' ') || timezone;
    return `${timezone.replace(/_/g, ' ')} - ${cityName}`;
  }
};
...
export const getTimezoneOptions = (): TimezoneOption[] => {
  ...
  // Sort by UTC offset and then by name
  return options.sort((a, b) => {
    // Extract UTC offset from label for sorting
    const offsetA = a.label.match(/UTC([+-]\d{2}:\d{2})/)?.[1] || '+00:00';
    const offsetB = b.label.match(/UTC([+-]\d{2}:\d{2})/)?.[1] || '+00:00';
