// Script to add 5 students to each class for test school 2
...
async function addStudent(studentData) {
  try {
    ...
    if (result.success) {
      ...
      console.error(`❌ Failed to add student: ${studentData.firstName} ${studentData.lastName}`, result);
      return false;
    }
  } catch (error) {
    console.error(`❌ Error adding student: ${studentData.firstName} ${studentData.lastName}`, error);
    return false;
  }
}
...
window.addTestStudents = addAllTestStudents;
window.addSingleStudent = addStudent;
window.testStudentsData = testStudents;

console.log('📚 Test students script loaded!');
console.log('Run addTestStudents() to add all students');
console.log('Run addSingleStudent(studentData) to add a single student');
console.log('Access testStudentsData to see all student data'); 