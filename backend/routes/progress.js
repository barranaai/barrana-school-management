...
async function validatedResults(body, ctx) {
  const deliveredObjectives = ctx.session.plannedSessionSnapshot?.objectives || [];
  const objectives = deliveredObjectives.some(o => o.objectiveId) ? deliveredObjectives : (ctx.planned.objectives || []);
  const objectiveId = o => o.objectiveId || o._id;
  const byId = new Map(objectives.filter(o => objectiveId(o)).map(o => [String(objectiveId(o)), o]));
  ...
  for (const r of objectiveResults) { const o = byId.get(String(r.objectiveId)); if (!o || seen.has(String(r.objectiveId))) return { error: 'Objective result does not match the Delivered Session snapshot' }; if (!['achieved','partially_achieved','not_achieved','needs_improvement','not_observed'].includes(r.status)) return { error: 'Invalid objective result status' }; seen.add(String(r.objectiveId)); objectiveSnapshots.push({ objectiveId: objectiveId(o), sequence: o.sequence, title: o.title, description: o.description, expectedOutcome: o.expectedOutcome, status: r.status, instructorNote: r.instructorNote, evidence: r.evidence, metadata: r.metadata }); }
  ...
  const plannedParameterIds = new Set(objectives.map(o => o.parameterId).filter(Boolean).map(String));
  if (ids.some(id => !plannedParameterIds.has(String(id)))) return { error: 'Parameter result does not match the Delivered Session snapshot' };
