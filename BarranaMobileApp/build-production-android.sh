...
# Production Android Build Script for Barrana Parent App
# Creates a standalone .apk file for distribution
...
echo ""
echo -e "${BLUE}🤖 ======================================${NC}"
echo -e "${BLUE}   Barrana Parent App - Android Build${NC}"
...
echo ""
echo "  1) ${GREEN}Production${NC}  - For Google Play Store"
echo "  2) ${YELLOW}Preview${NC}     - For internal testing"
...
case $choice in
    1)
        PROFILE="production"
        echo -e "${GREEN}✅ Building for Google Play Store (Production)${NC}"
        ;;
    2)
        PROFILE="preview"
        echo -e "${YELLOW}✅ Building for Internal Testing (Preview)${NC}"
        ;;
    3)
        PROFILE="development"
...
if [ "$PROFILE" = "production" ]; then
    echo "1. Download the .apk file from the build link"
    echo "2. Upload to Google Play Console"
    echo "3. Or submit via: ${BLUE}eas submit --platform android --latest${NC}"
elif [ "$PROFILE" = "preview" ]; then
    echo "1. Download the .apk file from the build link"
    echo "2. Share APK for internal testing"
    echo "3. Install on Android devices"
else
    echo "1. Download the .apk file from the build link"
    echo "2. Install on your test device"
    echo "3. Test all features thoroughly"
fi

echo ""
echo "📋 View build status: ${BLUE}eas build:list${NC}"
echo "📥 Download build:    ${BLUE}eas build:download --platform android${NC}"
echo ""
